...
Hardware upgrade (M5 Pro, 64GB unified memory) removed the constraints that shaped v0.4's model choices. See `IMPLEMENTATION_PLAN.md` for the full design rationale.

### Added
- **LLM semantic skill matching (Layer 3)**: after exact + fuzzy matching, still-unmatched JD terms plus the full resume skill list go to the LLM, which rules on EVERY term (`covered_by: <skill> | null`). Catches equivalence ("client relations" ≈ "customer service") and implementation ("GitHub Actions" → "CI/CD", "Prometheus" → "observability") — domain-agnostic, works for non-technical resumes. Guardrails: per-term verdict output (forced enumeration prevents both lazy empty output and volunteered false matches), verbatim subset validation (hallucinated matches structurally impossible), graceful degradation to fuzzy-only on any LLM failure. Result carries `semantic_matched` + `semantic_match_details` (`covered_by` + `reason`) for UI display.
- **`key_competencies` JD extraction field**: recruiter-scan concepts (microservices, DevOps, ML Ops, distributed computing, observability) extracted from anywhere in the posting — including prose responsibilities and section headings that previous versions never matched. Own `missing_competencies` + `competencies_match_pct` in the match result. Protected by the verbatim hallucination guard plus a deterministic people-filter (strips role/team terms like "AI Researchers", "business SMEs" the extractor kept mislabeling as competencies).
- **`LLM_GRADING_THINK` / `LLM_MATCHING_THINK`** env flags (both default `false`): thinking mode decoupled from model selection. `model_role` parameter added to `prompt_model()` — previously the `think` flag doubled as the model selector, making "big model without thinking" impossible.
- **`OLLAMA_KEEP_ALIVE`** (default `30m`) sent with every request to keep the model resident.
- Tests: 3-layer matching, hallucination guardrail, degradation, cache-hit, competency people-filter, `RUN_SLOW=1`-gated live Ollama integration test.

### Changed
- **Single model for all roles**: `qwen3:30b-a3b` (MoE, 3B active params) replaces the 4b/8b split. It generates at small-model speed (~93 tok/s on M5 Pro) with 32B-class quality, and keeping ONE model resident eliminates Ollama load-thrash — alternating 8b/30b caused 18GB reloads and multi-minute request stalls.
- **Grading thinking off by default**: hidden reasoning tokens (invisible in logged `eval_count`) were the real cause of 1.5–2 min grading calls. 30b-a3b without thinking outperforms the old 8b with thinking; grading now ~7s.
- Context window 8192 → 32768 (`OLLAMA_NUM_CTX`), warning threshold now 75% of window.
- `SKILL_ALIASES` frozen and trimmed to ambiguous abbreviations only (`k8s`, `tf`, `js`, …). Spelled-out synonyms ("postgres" → "postgresql") removed — fuzzy and semantic layers handle them. The map is no longer a coverage mechanism and never grows again.
- `compute_skill_match` is now async (Layer 3 calls the LLM).

### Fixed
- **Stale-cache bug**: cache keys for extraction, grading, and matching now include the model name, thinking flag, and system prompt text. Previously a model swap or prompt fix silently served results generated by the old model/prompt.
- Semantic matcher on the old 8b produced false positives (`kubernetes ← docker`, `gcp ← aws`, `security ← postgresql`) and occasionally malformed JSON keys — resolved by the 30b model plus the per-term verdict schema.
